...
这个方法利用 SSH 自带的“动态转发”功能，实现 SOCKS5 代理。

第 1 步：在慢速服务器上建立隧道
直接在你的慢速服务器终端运行：

```Bash
# 格式：ssh -D 本地端口 用户名@快服务器IP
ssh -D 1080 panyh@192.168.60.99
```
输入密码登录后，这个终端窗口不要关（或者你可以加上 -f -N 参数让它在后台跑）。

这就相当于在你的本地建立了一个通往 99 的秘密通道。

第 2 步：配置代理变量
现在，你的代理就在你自己的 localhost:1080 上了（虽然流量实际是走到了 99）：
...
局域网ip顾名思义就是局域网内部的ip地址。

公网ip一般是最后一步网关或者服务器对外展示的ip地址，再往外面走就是别家的服务器了。

网关通过 NAT 技术将设备的局域网ip和公网ip进行转换。

linux下查看局域网ip，`ifconfig`即可
