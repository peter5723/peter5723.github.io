...
链路层的信道很多，如局域网（LAN），无线局域网（WLAN），卫星网，光纤等广播信道，还有点对点通信信道，如两台路由器之间的通信链路。所以链路层有许多协议，如以太网、802.11 WLAN、令牌环、PPP 等。数据报在两个主机之间传输时，可以经过不同的链路，由不同的链路层协议承载，就好像不同的交通工具一样。
...
链路层是怎么实现的？在路由器上，我们已经看过。在主机上，链路层的主题是在网络适配器（Network adapter）中实现的，网络适配器也称为网卡（NIC，network interface card）。网络适配器的核心是链路层控制器，这个控制器上有许多芯片，用来实现链路层的各种服务（成帧、流量控制、差错检测等）。作为计算机的一个外设，今天的网卡大多都集成在了主板上，通过总线与 CPU 交互。下图是网络适配器与其他主机组件和协议栈功能的关系。
...
- 载波侦听多路访问协议（CSMA）：节点在发送数据前先侦听信道，如果信道空闲再发送，如果信道忙则等待。
