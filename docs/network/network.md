...
- 差错报告设施，用于报告数据差错和对某些网络层信息请求进行响应，如 ICMP 协议。
...
每个 IP 地址长度是 32 位，按字节分开。举例 IP：`192.168.30.2`。
...
一个地址的后 `32-x` 位用于区分组织的内部设备。比如子网掩码是 `255.255.255.0`，即 `x=24`，则后 8 位用于区分不同的设备。
